...
MODEL_XML_PATH = os.path.join("fetch", "reach_and_throw_with_path.xml")
MODEL_XML_PATH = os.path.join(os.path.dirname(__file__), "assets", MODEL_XML_PATH)


class FetchReachAndThrowWithPathEnv(fetch_env.FetchEnv, ut.EzPickle):
    def __init__(self, reward_type="sparse"):
        ...
        initial_qpos = {
            ...
            "robot0:slide1": 0.48,
            "robot0:slide2": 0.0,
            # "robot0:shoulder_lift_joint": - 50 * np.pi / 180,
    ...
    def _reset_sim(self):
        ...
        self.object_qpos[1] = np.array(0.75018422 + self.np_random.uniform(-0.01,0.01))     
    ...
    def compute_reward(self, achieved_goal, goal, info):
        ...
        if (self.success < 30):
            ...
            #-------------------------------------------------------------------
            if self.reward_type == "sparse":
                return -((self.object_qpos[2] > self.goal[2] + 0.1) |\
    ...
    def _is_success(self, achieved_goal, desired_goal):
        ...
        if (self.success < 30):
            ...
            if (((self.object_qpos[2] < self.goal[2] + 0.1) & (self.object_qpos[2] > self.goal[2] - 0.1) &\
                (self.object_qpos[0] > self.moving_point[0] - self.box_radius) &\
                    (self.object_qpos[0] < self.goal[0] + self.box_radius) &\
                        (self.object_qpos[1] > self.slope*(self.object_qpos[0]-self.goal[0]) + self.goal[1] - self.box_radius) &\
                            (self.object_qpos[1] < self.slope*(self.object_qpos[0]-self.goal[0]) + self.goal[1] + self.box_radius) &\
